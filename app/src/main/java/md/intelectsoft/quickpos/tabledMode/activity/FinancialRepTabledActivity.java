...
public class FinancialRepTabledActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cash_in.setOnClickListener(v -> {
            Shift shiftEntry = POSApplication.getApplication().getShift();
            ...
            if(shiftEntry != null ){
                ...
                final AlertDialog addPosition = new AlertDialog.Builder(FinancialRepTabledActivity.this).create();
...
//            alert11.show();
            ...
            if (cashInSafe[0] != null){
                Realm.getDefaultInstance().executeTransaction(realm -> {
                    Shift shift = realm.where(Shift.class).equalTo("id",POSApplication.getApplication().getShift().getId()).findFirst();
                    if(shift != null){
                        if(value > 0){
                            shift.setCashIn(shift.getCashIn() + value);
                            POSApplication.getApplication().getShift().setCashIn(shift.getCashIn());

                        }
                        else {
                            double absValue = Math.abs(value);
                            shift.setCashOut(shift.getCashOut() + absValue);
                            POSApplication.getApplication().getShift().setCashOut(shift.getCashOut());
                        }
                    }
                });
